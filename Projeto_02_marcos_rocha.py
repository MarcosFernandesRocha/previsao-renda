...
st.markdown('A maior parte das pessoas não possui filhos (mediana de 0). Em média, há 0,43 filhos por pessoa, com um desvio padrão de 0,75, e o máximo registrado é de 14 filhos.')
st.markdown('##### Idade:') 
st.markdown('A idade média é de aproximadamente 43,87 anos, com a maioria das pessoas variando entre 34 e 53 anos, intervalo interquartil. A idade mínima é 22 anos e a máxima, 68 anos.')
st.markdown('##### Tempo de Emprego:') 
st.markdown('A mediana do tempo de emprego é de cerca de 6 anos, com um tempo médio de 7,72 anos e desvio padrão de 6,72 anos. O intervalo interquartil varia de 3,01 a 10,12 anos, e o maior tempo registrado é de 42,91 anos.')
st.markdown('##### Quantidade de Pessoas na Residência:') 
st.markdown('A maior parte dos lares possui 2 pessoas, com uma média de 2,21 pessoas por residência e um desvio padrão de 0,91. O máximo registrado é de 15 pessoas.')
st.markdown('##### Renda:') 
st.markdown('A renda média é de 5.698,14 enquanto a mediana (de 3.488,41) é menor, indicando uma possível assimetria positiva nos dados , sendo que alguns valores muito altos elevam a média. O intervalo interquartil varia de 2.018,88 a 6.379,57, com renda mínima de 118,71 e máxima de 245.141,67.')
...
st.markdown('É possível observar que penas a variável ´tempo_emprego´ apresenta um índice considerável para análise. Além disso, foram apresentadas duas variáveis booleanas, ´posse_de_imovel´ e ´posse_de_veiculo´, onde a variável que possui mais relação com a renda é a de posse de veículo.')
...
st.markdown('É possível identificar facilmente a covariância positiva com a inclinação da linha de tendência, podemos afirmar então que existe uma relação entre os dados de renda e tempo de emprego.')
...
st.markdown('A análise das variáveis qualitativas ao longo do tempo evidencia a correlação da variável `renda` com a variável de `posse_de_veiculo`. Outra variável que mostra um mudança ao longo do tempo é o aumento do número de pessoas com um Ensino superior completo. As demais variáveis não representam correlação relevante.')
...
st.markdown('''
##### Visualização dos resultados de previsão <a name="clustermap"></a>
''', unsafe_allow_html=True)

previsao_2 = renda[['renda', 'renda_predict']]
st.line_chart(previsao_2)
...
st.markdown('Na execução deste modelo seria interessnate adicionar locais de imputação de dados relevantes para a base para que se possa prever a renda do cliente com base no modelo. Variáveis como o tempo de emprego, posse de veículo, sexo, quantidade de filhos ou/e tipo de renda seriam importantes para o modelo. Com isso pode-se criar automaticamete concessão e limites de crédito, além de poder oferecer produtos e ofertas especiais de acordo com cada linha de crédito por exemplo.')
